...
<form action="options-general.php?page=one-email-only/options/index.php&subscribepanel=1" method="post" id="update_address_form"
...
<form action="options-general.php?page=one-email-only/options/index.php&subscribepanel=1" method="post">
...
if (!empty($clean_email)){
	...
	if ($starting_from > 0){
		...
		echo "<a href='options-general.php?page=one-email-only/options/index.php&amp;subscribepanel=1&amp;starting=$new_starting&amp;sre=$clean_email&amp;srt=$search_type'>".__('&laquo; Previous','subscribe-reloaded')."</a> ";
	}
	if (($ending_to < $count_total) && ($count_results > 0)){
		$new_starting = $starting_from+25;
		echo "<a href='options-general.php?page=one-email-only/options/index.php&amp;subscribepanel=1&amp;starting=$new_starting&amp;sre=$clean_email&amp;srt=$search_type'>".__('Next &raquo;','subscribe-reloaded')."</a> ";
	}
} ?></p>
...
<p><?php printf(__('You can either <a href="%s">list all the subscriptions</a> or find those where the <b>email</b>','subscribe-reloaded'), 
	'options-general.php?page=one-email-only/options/index.php&subscribepanel=1&sre=@&srt=c') ?>&nbsp;
<select name="srt">
...
<form action="options-general.php?page=one-email-only/options/index.php<?php if(!empty($current_panel)) echo '&subscribepanel='.$current_panel; ?>" method="post" id="post_list_form"
	onsubmit="return confirm('<?php _e('Please remember: this operation cannot be undone. Are you sure you want to proceed?', 'subscribe-reloaded') ?>')">
<fieldset style="border:0">
<?php
	...
	if (is_array($subscriptions) && !empty($subscriptions)){
		...
		echo '<ul>';
		$order_dt = "<a style='text-decoration:none' title='".__('Reverse the order by Date/Time','subscribe-reloaded')."' href='options-general.php?page=one-email-only/options/index.php&amp;subscribepanel=1&amp;sre=$clean_email&amp;srt=$search_type&amp;srob=dt&amp;sro=".(($order=='ASC')?"DESC'>&or;":"ASC'>&and;")."</a>";
		$order_status = "<a style='text-decoration:none' title='".__('Reverse the order by Date/Time','subscribe-reloaded')."' href='options-general.php?page=one-email-only/options/index.php&amp;subscribepanel=1&amp;sre=$clean_email&amp;srt=$search_type&amp;srob=status&amp;sro=".(($order=='ASC')?"DESC'>&or;":"ASC'>&and;")."</a>";
		$order_post_id = "<a style='text-decoration:none' title='".__('Reverse the order by Post ID','subscribe-reloaded')."' href='options-general.php?page=one-email-only/options/index.php&amp;subscribepanel=1&amp;sre=$clean_email&amp;srt=$search_type&amp;srob=post_ID&amp;sro=".(($order=='ASC')?"DESC'>&or;":"ASC'>&and;")."</a>";
		...
		foreach($subscriptions as $a_subscription){
			...
			$specific_email = (($search_type == 'c') || ($search_type == 'n'))?"<a href='options-general.php?page=one-email-only/options/index.php&subscribepanel=1&sre=$a_subscription->email&srt=e'>$a_subscription->email</a> ":'';
