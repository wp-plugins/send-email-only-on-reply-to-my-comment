...
Author: Yasir
Author URI: http://www.a1newspapers.com/
*/
...
function subscribe_reloaded_show(){
	...
	if ($show_subscription_box){
		...
		$checkbox_label = str_replace('[subscribe_link]', "$manager_link&amp;sra=s", 
			stripslashes(get_option('subscribe_reloaded_checkbox_label', __("Notify via Email Only if someone replies to My Comment",'subscribe-reloaded'))));
...
class wp_subscribe_reloaded {
	...
	public function __construct(){
		...
		add_action('admin_print_styles-one-email-only/options/index.php', array( &$this, 'add_options_stylesheet') );
	...
	public function activate(){
		...
		add_option('subscribe_reloaded_manager_page_title', __('Unsubscribe','subscribe-reloaded'), '', 'no');
		add_option('subscribe_reloaded_request_mgmt_link', __('Please enter your email address so that we can send you a Link using which you will be able to Unsubscribe:', 'subscribe-reloaded'), '', 'no');
		add_option('subscribe_reloaded_request_mgmt_link_thankyou', __('Thank you for using our subscription service. Your request has been completed, and you should receive an email with the Unsubscribe link in a few minutes.', 'subscribe-reloaded'), '', 'no');
		...
		add_option('subscribe_reloaded_author_text', __("Please select the Article(s) from which you want to unsubscribe. If you want to unsubscribe from all articles, then please click on Select All: and then click on Unsubscribe Button", 'subscribe-reloaded'), '', 'no');
		add_option('subscribe_reloaded_user_text', __("Please select the Article(s) from which you want to unsubscribe. If you want to unsubscribe from all articles, then please click on Select All: and then click on Unsubscribe Button:", 'subscribe-reloaded'), '', 'no');
		...
		add_option('subscribe_reloaded_notification_content', __("There is a new comment to [post_title].\nComment Link: [comment_permalink]\nAuthor: [comment_author]\nComment:\n[comment_content]\nPermalink: [post_permalink]\nTo Unsubscribe go here: [manager_link]",'subscribe-reloaded'), '', 'no');
		add_option('subscribe_reloaded_checkbox_label', __("Notify via Email Only if someone replies to My Comment",'subscribe-reloaded'), '', 'no');
		...
		add_option('subscribe_reloaded_management_content', __("You have requested to Unsubscribe from [blog_name]. Follow this link to access the required page: [manager_link]",'subscribe-reloaded'));
	...
	public function new_comment_posted($_comment_ID = 0, $_comment_status = 0){
		...
		$child1 = $child1[0];
			...
			$parent1 = mysql_query("SELECT `comment_author_email` FROM $wpdb->comments WHERE `comment_ID` = '$child1'");
	...
	public function subscribe_reloaded_manage($_posts = '', $_query = ''){
		...
		if (!empty($action) && ($action == 's') && ($post_ID > 0)){
			$include_post_content = include(WP_PLUGIN_DIR.'/one-email-only/templates/subscribe.php');
		}

		// Management page for post authors
		elseif (($post_ID > 0) && $this->is_author($target_post->post_author)){
			$include_post_content = include(WP_PLUGIN_DIR.'/one-email-only/templates/author.php');
		}
		...
		elseif ( ($post_ID > 0) && !empty($email) && !empty($key) && !empty($action) &&
				...
				($action = 'c') ){
			$include_post_content = include(WP_PLUGIN_DIR.'/one-email-only/templates/confirm.php');
		}

		// Manage your subscriptions (user)
		elseif ( !empty($email) && ((!empty($key) && $this->_is_valid_key($key, $email)) || current_user_can('read')) ){
			$include_post_content = include(WP_PLUGIN_DIR.'/one-email-only/templates/user.php');
		}

		if (empty($include_post_content))
			$include_post_content = include(WP_PLUGIN_DIR.'/one-email-only/templates/request-management-link.php');
	...
	public function add_config_menu( $_s ) {
		...
		if (current_user_can('manage_options')){
			add_options_page( 'One Email', 'One Email', 'manage_options', WP_PLUGIN_DIR.'/one-email-only/options/index.php' );
		}
		return $_s;
	}
	// end add_config_menu
	...
	public function add_options_stylesheet() {
		...
		$stylesheet_url = (is_ssl()?str_replace('http://', 'https://', WP_PLUGIN_URL):WP_PLUGIN_URL).'/one-email-only/style.css';
	...
	public function add_post_comments_stylesheet() {
		// It looks like WP_PLUGIN_URL doesn't honor the HTTPS setting in wp-config.php
		$stylesheet_url = (is_ssl()?str_replace('http://', 'https://', WP_PLUGIN_URL):WP_PLUGIN_URL).'/one-email-only/post-and-comments.css';
		wp_register_style('subscribe-to-comments', $stylesheet_url);
		wp_enqueue_style('subscribe-to-comments');
	}
	// end add_stylesheet
	...
	public function add_column($_column_name){
		...
		if ($this->is_user_subscribed($comment->comment_post_ID, $comment->comment_author_email, 'Y'))
			echo '<a href="admin.php?page=one-email-only/options/index.php&subscribepanel=1&sre='.urlencode($comment->comment_author_email).'">'.__('Yes','subscribe-reloaded').'</a>';
