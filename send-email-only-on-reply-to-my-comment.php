...
$one1="1";
	...
	public function add_config_menu( $_s ) {
		...
		if (current_user_can('manage_options')){
			add_options_page( 'One Email', 'One Email', 'manage_options', WP_PLUGIN_DIR.'/send-email-only-on-reply-to-my-comment/options/index.php' );
		}
		return $_s;
	}
	// end add_config_menu
	...
	public function add_column($_column_name){
		...
		if ($this->is_user_subscribed($comment->comment_post_ID, $comment->comment_author_email, 'Y'))
			echo '<a href="admin.php?page=send-email-only-on-reply-to-my-comment/options/index.php&subscribepanel=1&sre='.urlencode($comment->comment_author_email).'">'.__('Yes','subscribe-reloaded').'</a>';
