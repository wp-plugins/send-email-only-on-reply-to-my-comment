...
if ($offset > 0){
	...
	$previous_link = "<a href='options-general.php?page=one-email-only/options/index.php&amp;subscribepanel=1&amp;srsf=$new_starting&amp;srrp=$limit_results&amp;srv=$search_value&amp;srt=".urlencode($search_type)."&amp;srob=$order_by&amp;sro=$order'>".__('&laquo; Previous','subscribe-reloaded')."</a> ";
}
if (($ending_to < $count_total) && ($count_results > 0)){
	$new_starting = $offset+$limit_results;
	$next_link = "<a href='options-general.php?page=one-email-only/options/index.php&amp;subscribepanel=1&amp;srsf=$new_starting&amp;srrp=$limit_results&amp;srv=$search_value&amp;srt=".urlencode($search_type)."&amp;srob=$order_by&amp;sro=$order'>".__('Next &raquo;','subscribe-reloaded')."</a> ";
}